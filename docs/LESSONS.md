...
- **Supabase 场次回填（2026-09-15）**：`shows.date` 是 text，而 `ratings.performance_date` 是 date。跨表回填时必须先用 `^[0-9]{4}-[0-9]{2}-[0-9]{2}$` 校验，再在 `case` 分支中显式 `::date`；不能直接把 text 赋给 date，也不要假设同步数据永远格式正确。真实场次的时段也必须统一由 `rating_session_from_show_time(shows.time)` 判断：00:00–15:59 为午场、16:00–17:59 为夕场、18:00 后为晚场。前端只提供辅助信息，避免两端分类规则漂移。
