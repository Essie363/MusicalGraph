...
- **Supabase 文本日期迁移（2026-09-15）**：`shows.date` 是 text，而 `ratings.performance_date` 是 date。跨表回填时必须先用 `^[0-9]{4}-[0-9]{2}-[0-9]{2}$` 校验，再在 `case` 分支中显式 `::date`；不能直接把 text 赋给 date，也不要假设同步数据永远格式正确。新评分 RPC 以 `shows` 中的场次日期为准，前端日期仅作辅助信息。
